...
**Description:** The table stores history for ECA_OBJECT table.
...
- **IF_ECA_OBJECT_HISTORY_ECA** on `EcaID`
- **IF_ECA_OBJECT_HISTORY_LOT_NO** on `LotNo, ProductID`
- **IF_ECA_OBJECT_HISTORY_ORDER_DETAIL** on `OrderNo, OrderType, OrderLineNo`
- **IF_ECA_OBJECT_HISTORY_PRODUCT** on `ProductID`
- **IF_ECA_OBJECT_HISTORY_RESOURCE** on `ResourceID`
- **IF_ECA_OBJECT_HISTORY_SERIAL_NO** on `SerialNo, ProductID`
- **IF_ECA_OBJECT_HISTORY_UNIT** on `UnitID`
- **IF_ECA_OBJECT_HISTORY_WIP_OPERATION_STEP** on `WipOrderNo, WipOrderType, OprSequenceNo, StepSequenceNo`
