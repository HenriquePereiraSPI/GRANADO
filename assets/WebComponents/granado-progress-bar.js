  ...
  _build() {
    ...
    const renderSlot = (items, slotName) => {
      ...
      const itemsHtml = items.map(it => {
        ...
        if (it === 'value') return `<span data-pb-value style="font-weight:700;font-family:ui-monospace,SFMono-Regular,Menlo,monospace"></span>`;
        return '';
      }).join('');
      return `<div data-pb-slot="${slotName}" style="display:flex;align-items:center;${justify}gap:8px;font-size:11px;color:#103E20;font-family:system-ui,-apple-system,sans-serif;line-height:1.2">${itemsHtml}</div>`;
    };
