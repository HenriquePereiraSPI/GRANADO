# CreateDispositionFromInspectionPlan
...
This Business Component method is used to create Disposition record that represents the header of the quality inspection. It has similar set of inputs as CreateDispositon BC plus additional ones. Based on the Inspection Plan structure it creates a new Disposition with lines and disposition tests attached.
...
- Creates Disposition based on the inputs specified (the same as for CreateDisposition plus InspectionPlanID will be persisted in DISPOSITION table). 
...
- If QualityGateID and QualityGateSeqItemID are specified for the Inspection Plan, these values are copied for the created Disposition.
...
|  | InspectionPlanID | InspectionPlanIDInspection |
|  | QualityGateID | Value copied from INSPECTION_PLAN. |
|  | QualityGateSeqItemID | Value copied from INSPECTION_PLAN. |
