...
This Business Component method is used to create a Disposition based on existing one passed as a parameter which includes: creation of the new Disposition record and duplication of all Disposition Lines and Tests assigned to the Disposition being duplicated. The difference between this BC method and DuplicateDispositon_v2 is that it has three additional inputs: Rework, ReferenceDispositionNumber and ReferenceFacility.

## Parameters

| IO | Name | Type | Required | Description |
|----|------|------|----------|-------------|
| Input | ReferenceDispositionNumber | Char | Yes | Number of an original Disposition (the one that is duplicated). |
| Input | ReferenceFacility | Char | Yes | Facility where the Inspection of the original Disposition (the one that is duplicated) takes place. |
...
| Input | PartnerRole | Integer | No | Role of Customer or Vendor to which the Quality Defect is assigned. |
...
- System validates if SampleProcedureID exists in SamplingProcedure_table. 
...
|  | ReferenceFacility | Inputted Facility |
