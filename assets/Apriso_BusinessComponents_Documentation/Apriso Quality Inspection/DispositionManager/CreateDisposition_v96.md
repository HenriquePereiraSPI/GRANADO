...
This Business Component method is used to create Disposition record that represents the header of the Quality Inspection. The difference between this BC method and CreateDispositon_v2 is that it has additional input: PartnerRole.
...
| Input | ReferenceDisposition | Char | No | Number of an original Disposition (the one that is duplicated). |
| Input | ReferenceFacility | Char | No | Facility where the Inspection of the original Disposition (the one that is duplicated) takes place. |
...
- If WipOrderNo is provide, it must exist in WIP_ORDER table. 
- If PartnerID is provided, it must exist in PARTNER table.  
