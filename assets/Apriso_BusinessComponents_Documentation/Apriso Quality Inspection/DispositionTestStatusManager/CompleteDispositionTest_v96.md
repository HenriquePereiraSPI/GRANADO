...
| Input | Conforming | Integer | Yes | Indicates if the Disposition Test conforms to the specification (0 - non-conforming, 1 - conforming; any number other than 0 or 1 treated as not specified). |
...
| Input | Force | Boolean | Yes | If set to True, this causes the status to be changed for all the child entities (Samples) without performing additional validation (Propagate flag for Change Disposition Test Status is set to True). |
...
- If Reading Validation = Can be smaller (value of 3), the system checks if NoOfReadings >= the number of active Disposition Reading records. 
- If Reading Validation = Can be larger (value of 4), the system checks if NoOfReadings <= the number of active Disposition Reading records. 
 
- The system validates that if the Required flag (DISPOSITION_TEST.Required) is set to True and DispositionTestSampleID is not passed, the Test Value (or Test Attribute in case of an Attribute Characteristic) of the Disposition Test is recorded: 
...
- If DispostionTestSampleID is not passed, the system does the following: 
...
- If RecordResultsOnSamples flag = True, then when completing the test or sample, the system checks if the sample is recorded excluding records on the Disposition Test level.  
