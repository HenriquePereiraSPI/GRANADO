...
| Input | UHL | Decimal | No | Default defect code when the value of the characteristic is above the Upper Specification Limit (applicable only for characteristics of type: Variable). |
...
| Input | ResourceType | Short | No | Type of resource that is allowed to be assigned to disposition test |
| Input | ResourceClassID | Integer | No | Class of resource that is allowed to be assigned to disposition test. |
