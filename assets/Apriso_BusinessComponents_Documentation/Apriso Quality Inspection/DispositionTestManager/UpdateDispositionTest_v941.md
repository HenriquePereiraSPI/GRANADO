...
| Input | LHL | Decimal | No | Lower Coherence Limit. NULL = Decimal.MinValue. |
...
| Input | UHL | Decimal | No | Upper Coherence Limit. NULL = Decimal.MinValue. |
| Input | USL | Decimal | No | Upper Specification Limit. NULL = Decimal.MinValue. |
| Input | UHLDefectCode | Char | No | Default defect code when the value of the characteristic is above the Upper Coherence Limit (applicable only for characteristics of type: Variable). |
...
| Input | ResourceType | Short | No | Type of resource that can be assigned to disposition test |
| Input | ResourceClassID | Integer | No | Class of resource that can be assigned to disposition test |
...
- If UHLDefectCode is specified then does the same validation as for USLDefectCode 
- If LHLDefectCode is specified then does the same validation as for USLDefectCode 
- If ResourceType and/or ResourceClassID is specified then validates that the resources already assigned to disposition test and all of its disposition test samples are of that type and class. 
- Updates Disposition Test record passing all input parameters - if a parameter is not specified then the corresponding value in the DB is set to NULL.
...
|  | UpperCoherenceLimit | Inputted UHL or Null if Decimal.MinValue |
|  | LowerCoherenceLimit | Inputted LHL or Null if Decimal.MinValue |
...
|  | LHLDefectReasonCode | Inputted LHLDefectCode or null if not specified |
|  | UHLDefectReasonCode | Inputted UHLDefectCode or null if not specified |
|  | ResultsOnSamples | Inputted RecordResultsOnSamples |
|  | DefectCode | Inputted DefectCode or Null if not specified |
|  | ResourceType | Inputted ResourceType or Null if less or equal 0 |
|  | ResourceClassID | Inputted ResourceClassID or Null if less or equal 0 |
|  | ResultsOnSamples | Inputted RecordResultsOnSamples |
