...
- If USLDefectCode, LSLDefectCode, DefectCode not empty then exist in REASON_CODE table and are of type Defect Codes (REASON_CODE.ReasonType = 25) 
- If Resources list is not empty then for every item Resource exists in RESOURCE table and is of type Equipment (exists in EQUIPMENT) 
...
- USL <= UCL <= LCL <= LSL (value of Decimal.MinValue treated as the parameter is not specified) 
...
|  | USLDefectCode | USLDefectCode |
|  | LSLDefectCode | LSLDefectCode |
|  | DefectReasonCode | DefectCode |
|  | ResultsOnSamples | RecordResultsOnSamples |
