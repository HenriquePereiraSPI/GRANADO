...
| Input | LineCode | Char | No | Line code to be updated. |
...
| Input | SamplingProcedureID | Integer | No | Identifier of the default SamplingProcedureID used for the quality inspectionn. |
...
- SamplingProcedureID exists in SAMPLING_PROCEDURE table. 
- WorkCenter exists in WORK_CENTER table. 
...
|  | SamplingProcedureID | Inputted SamplingProcedureID or Null if not specified. |
|  | LineCode | Inputted LineCode or Null if not specified. |
|  | WorkCenter | Inputted WorkCenter or Null if not specified. |
