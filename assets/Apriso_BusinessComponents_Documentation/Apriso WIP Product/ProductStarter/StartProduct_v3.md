...
- If an EmployeeID is specified, then the system will create an open labor record in the LABOR table if the employee is labor-tracked 
- If the employee is also attendance-tracked, the system will ensure there is an attendance record for this time and will associate the created labor with that attendance
...
TRANSACTION_HISTORY - Transaction Name: FlexNet.BusinessFacade.Manufacturing.ProductStarter TRANSACTION_HISTORY_RESOURCE
