...
|----|------|------|----------|-------------|
| Input | OrderNo | Char | Yes | order number of the line. |
| Input | OrderType | Integer | Yes | order type of the line. |
| Input | OrderLineNo | Integer | Yes | order line nummber. |
| Input | OrderLineStatus | Integer | Yes | status of the line. |
...
- If the TriggerReexplosion Flag is set to yes, then re-explosion is triggered for all the children Work Orders. 
