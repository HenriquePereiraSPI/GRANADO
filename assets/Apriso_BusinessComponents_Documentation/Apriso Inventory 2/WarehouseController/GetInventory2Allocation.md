...
This Business Component method reads a record from INVENTORY2_ALLOCATION table for specified input parameters. When no record or multiple records are found, an error is returned.
...
If the record does not exist or input criteria are ambiguous (multiple records are found), an error is returned.
