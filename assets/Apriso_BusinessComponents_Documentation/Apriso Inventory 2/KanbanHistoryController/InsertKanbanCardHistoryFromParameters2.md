# InsertKanbanCardHistoryFromParameters2
...
Creates Kanban Card history based on input parameters with data taken from the KANBAN_CARD table. This version supports additional parameters Lot No and Product Revision.
...
| Input | TransactionToken | Char | Yes | The user transaction token number. |
...
| Input | KanbanCardStatusName | Char | Yes | The Kanban Card status name. |
...
|  | LotNo | Copied from LotNo input parameter. |
|  | ProductRevision | Copied from ProductRevision input parameter. |
