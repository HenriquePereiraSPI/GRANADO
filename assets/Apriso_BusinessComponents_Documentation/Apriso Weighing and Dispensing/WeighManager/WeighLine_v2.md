...
This method performs weighing of the Weigh Line. The method uses Inventory 2 data model.
...
- System moves Destination Container to the Destination Warehouse Location if needed (using MoveInventory2ByContainer BC). 
- If Net Weight exceeds available inventory, system increases the source inventory (using IncreaseNotAllocatedInventory2 BC). 
- If the source Container and Destination Container are the same, and Net Weight is less than available quantity, system decreases (scraps) remaining source inventory (using DecreaseNotAllocatedInventory2 BC). 
