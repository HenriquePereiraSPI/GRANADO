...
This method performs weighing by Direct Introduction of Containers. The method uses Inventory 2 data model.
...
- System moves Destination Container to Destination Warehouse Location if both are provided (using MoveInventory2ByContainer BC). 
- If Mode is "DIPOUR": 
 
- System moves the content from the source Container to Destination Container (using MoveNotAllocatedInventory2 BC for all inventory in that container). 
- System removes the source Container from the source Weigh Header. 
- System adds Destination Container to the destination Weigh Header (a parent of Weigh Line). 
 
- If Mode is "DIJOIN": 
 
- System moves the source Container to Destination Warehouse Location (using MoveInventory2ByContainer BC). 
- System removes the source Container from the source Weigh Header. 
