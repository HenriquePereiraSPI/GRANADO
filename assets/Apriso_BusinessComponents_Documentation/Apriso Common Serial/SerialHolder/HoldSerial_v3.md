...
| Input | FutureHoldName | Char | No | Name of the FutureHold that puts on Hold the item of a Containment. |
...
- The Containment ID if input is validated by checking if the Containment exists. If the item is not in containment then it an error. In the Serial Hold record the input ContainmentID is set (only for the input serial). 
