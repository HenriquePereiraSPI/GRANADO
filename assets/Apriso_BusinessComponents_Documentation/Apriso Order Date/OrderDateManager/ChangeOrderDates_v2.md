...
| Input | OrderType | Short | Conditional | The order type for which to create or update the order date. This Input requires the OrderNo to also be supplied. |
| Input | OrderLineNo | Integer | No | The order line number for which to create or update the order date. This Input requires the OrderNo and the OrderType Inputs to also be supplied. |
| Input | WipOrderNo | Char | Conditional | The WIP Order number for which to create or update the order date. |
| Input | WipOrderType | Short | Conditional | The WIP Order type for which to create or update the order date. This Input requires the WipOrderNo to also be supplied. |
| Input | OprSequenceNo | Char | No | The Operation Sequence Number for which to create or update the order date. This Input requires the WipOrderNo and the OrderType Inputs to also be supplied. |
...
- The system checks if the order date for the inputted entity already exists. If it does, the system updates the date for that order date record. Otherwise, the system will create a new record.
...
| ORDER_DATE | OrderType | OrderType (or WipOrderType if the inputted order is an Execution Order). |
| ORDER_DATE | WipOrderNo | WipOrderNo (if the inputted order is an Execution Order). |
| ORDER_DATE | OprSequenceNo | OprSequenceNo (if the inputted order is an Execution Order). |
