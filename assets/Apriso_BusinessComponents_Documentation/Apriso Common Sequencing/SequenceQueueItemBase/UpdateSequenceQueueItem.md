...
| Input | OrderNumber | Char | No | Number of the order linked to the queued item. |
...
| Input | ProductID | Integer | No | Product linked to queued item. |
...
| Input | SequenceQueueItemID | Integer | No | Item status. |

## System Processing

- Sequence queue item id is validated. If it is less than equal to 0, it errors out. 
- If a sequence queue item is found for the given id, an error message is returned. 
