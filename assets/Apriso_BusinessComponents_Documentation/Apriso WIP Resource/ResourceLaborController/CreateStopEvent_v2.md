...
| Input | ProductionLine | Char | No | The production line of the resource for the event |
...
|------|------|-------------|
| StartTimeUtc | DateTime | The UTC start time for the event (if provided, it will be used instead of the StartTime input). The records are saved with milliseconds accuracy. |
| StopTimeUtc | DateTime | The UTC stop time for the event (if provided, it will be used instead of the StopTime input). The records are saved with milliseconds accuracy. |

## Validations

- System checks if the appropriate master records exist in DELMIA Apriso when the ResourceName, ResourceType, Production Line, EmployeeID, Reason Code and SpeedUOM, Occupation are specified. 
