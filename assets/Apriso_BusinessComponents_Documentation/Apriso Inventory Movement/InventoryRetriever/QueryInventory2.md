...
This Business Component method reads a record from the INVENTORY2 table using input criteria. It returns an error when multiple rows are found (i.e. the Inventory is not unique).
...
- System reads a record from the INVENTORY2 table using input criteria. An error is returned when multiple rows are found. When Inventory is not found, the BC returns 0.
