...
| Input | DFCRevision | Char | Conditional | The DFC revision for the transaction history. |
...
- The system checks that DFCCode is non-empty (if DFCRevision is provided).  
...
The difference between the aggregated and the full mode is visible only for such Container actions as allocate, deallocate, decrease and move with child containers. ChangeInventory2Properties_v2 updates specific row from INVENTORY2 table (based on Inventory2ID provided as input).
