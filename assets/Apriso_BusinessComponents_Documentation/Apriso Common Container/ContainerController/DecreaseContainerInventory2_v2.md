# DecreaseContainerInventory2_v2
...
| Input | ProjectCode | Char | No | ProjectCode for transaction history. |
...
- Container and its sub-Containers are allocated only to a given AllocationID, or not allocated when AllocationID is not provided, 
- StartDate is before EndDate (if they are provided), 
...
- System removes the INVENTORY2 records for the specified Container and its sub-Containers. 
...
|  | InventoryAllocationID | System checks if AllocationID matches. |
