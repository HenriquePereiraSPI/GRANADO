...
Merges one or more PDF documents in to single combined document. Any source document that fails validation is not merged. The component does not fail unless all source documents fail to merge. The component is limited to the files in the report export directory (CentralConfiguration -> FlexNet.Reports -> ReportExportDir) or child directories.

## Parameters

| IO | Name | Type | Required | Description |
|----|------|------|----------|-------------|
| Input | DestinationFilePath | Char | Yes | Relative file path of the combined PDF document. |
| Input | SourceFilePaths | List of Char | Yes | Relative file paths of the source PDF documents. |
...
- Validation fails if a source or destination file path is not under the report export directory. 
