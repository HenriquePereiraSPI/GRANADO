...
Returns the dynamically calculated ID of a DFC revision that meets effectivity/discontinue date conditions. The DFC must be in the prototype or active status and must have the correct compilation status. If more than one revision of the DFC meets these conditions, the ID of the default DFC revision is returned.
...
| Output | operationID | Integer | Yes | ID of a DFC which meets all conditions. |

## System Processing

- System searches for a DFC inside of Project if the projectCode Input is provided. 
