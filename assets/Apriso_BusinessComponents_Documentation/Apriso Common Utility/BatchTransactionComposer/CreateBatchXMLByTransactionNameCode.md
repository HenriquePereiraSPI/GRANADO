# CreateBatchXMLByTransactionNameCode
...
|----|------|------|----------|-------------|
| Input | TransactionName | Char | No | The Name of the Transaction of which the caller wants to group. |
| Input | TransactionCode | Char | No | The Transaction Code of which the caller wants to group. |

## System Processing

System reads all records from Transaction_Data_Batch_Process table that have the values of TransactionName and TransactionCode specified from the input parameter. Then group the records together by user configured criteria and creates a batched XML for each group. A new Job Executor Job is created for each batched XML which will be further process by Business Integrator to external system.
