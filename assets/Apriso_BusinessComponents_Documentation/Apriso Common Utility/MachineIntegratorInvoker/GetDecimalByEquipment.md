...
The purpose of this Business Component method is to get the Decimal value of the Machine Integrator Point assigned to a specific Equipment's Feature.
...
| Output | Value | Decimal | No | The value retrieved from the machine Point. |
...
The Equipment and Feature assignment to the Machine Integrator Point is validated and if not found, the "ErrorUnknownAlias" ErrorCode is set In case of a communication error or no connection to Machine Integrator Connector, the "NoConnection" ErrorCode is set If an assigned Point has a different type than expected, then the "ErrorBadType" ErrorCode is set If no providers are found for the requested items or an exception occurs, the error message is displayed
