...
**Status:** Active
**Keywords:** CAPA, NC, issue, incident, corrective, preventive, non-conformity, non-compliant, Task, CAPATask, Delete

## Description

The purpose of this Business Component method is to delete an existing record from the CAPA_TASK table.
...
- The system validates that CAPATaskID is provided. 
- The system validates that the provided CAPATaskID exists in the CAPA_TASK table.

## System Processing

- The system deletes the record from the CAPA_TASK table. 
- If child records exist in the CAPA_TASK_STATE table, the system deletes them. 
- If the deleted task was in a flow that is linked to a CAPA (is used in runtime), the system writes historical information about the changes to the CAPA_HISTORY table, unless the DisableHistory input is set to true.
