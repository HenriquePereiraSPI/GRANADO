...
|----|------|------|----------|-------------|
| Input | UnitID | Integer | Yes | Unit ID to crate unit characteristic for. |
...
| Input | Value | Decimal | No | Decimal. |
...
- verifies that inputted Characteristic and Unit exist. If record is not found, System generates an error if not retrieved; 
- verifies that for values passed as input, the UOM is populated.
...
- If UOM is passed as input, but other values are copied from the characteristic, then the copied values are converted in the input UOM. This is done by invoking the Business Component UOMConvert. 
