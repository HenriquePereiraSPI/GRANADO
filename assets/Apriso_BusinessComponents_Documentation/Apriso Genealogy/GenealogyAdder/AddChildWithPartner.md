...
| Input | ChildLotNo | Char | Conditional | The child Lot Number. This is required if a child product is lot-tracked or both lot-tracked and serial-tracked. |
...
| Input | LotInternalOrExternalFlag | Boolean | Yes | If true, then the child Lot Number is treated as an internal Lot Number. Otherwise, it is treated as an external supplier Lot Number. |
| Input | ChildPartnerID | Integer | Conditional | The child's partner ID. If LotInternalOrExternalFlag is false, then it is required. |
| Input | UomCode | Char | Yes | The child's unit of measure code. |
| Input | Quantity | Decimal | No | The child's quantity to be added. This must be greater than or equal to 0. |
...
- The system validates the child product number that is to be added. The product must be valid in the system, and the product is not on hold. 
...
- If LotInternalOrExternalFlag is false, ChildPartnerID must be valid for this Lot Number. 
 
- If the child is serial-tracked, the system validates the entered child Serial Number: 
