...
This Business Component method increments the utilization counters for all Maintenance Procedures linked to a Resource. It can be used to increment utilization counters for all Equipment in a given Work Center or a Resource.
...
When Work Center is not specified and ResourceID is specified, it increments utilization counters for the specified Resource. If the cascade flag is set to true, it also updates all Resources linked to the specified Resources through *resource_resource_link/resource_collection *
...
-  System validates if UtilizationMode for updating Resource is set to Counter.  
 
-  If UtilizationMode for updating Resource is set to Actual then the system ignores this mode and does not process it.  
...
- System updates the Counter value for all Resources from the list.
