...
| Output | Median | ListofDecimal | No | The median value of all data points on the chart. |
...
- The system validates ChartID. If it has not been specified, the system returns an error. If it has been specified, the system validates whether it is a valid ChartID in the SPC_CHART table. If it is not valid, the system returns an error. If it is valid, the system validates whether it has an SPCChartType associated with it. If it does not have a SPCChartType associated with it, the system returns an error. If it has an SPCChartType associated with it, system uses the ChartID for calculation. 
- The system validates Query. If it has been specified, the system uses Query for the calculation. If it has not been specified, the system uses the CalculateQuery field in the SPC_CHART table. If there is no CalculateQuery field in the SPC_CHART table, the system uses the CalculateQuery field in the SPC_CHART_TYPE table. If there is no CalculateQuery field in the SPC_CHART_TYPE table, the system returns an error.  
 
- The system substitutes tags for real values. 
...
Otherwise, LCLValue1 = XBar - (Ratio1 * XBar) where Ratio1 is a value dependent on subgroup size.
...
Average of measurements = sum of measurements/otal number of measurements.
...
If both the UpperControlLimit and LowerControlLimit fields from the SPC_CHART_PROPERTIES table are not NULL, UCLValue2 = the UpperControlLimit value from SPC_CHART_PROPERTIES table.
...
For the first record, PlottedValue1 = the subgroup average = totals of the subgroup/subgroup size. Otherwise, PlottedValue1 = (λ * subgroup average) + ((1 - λ) * previous PlottedValue1)
 

where λ = the WeightingFactor value from the SPC_CHART table (0< λ<=1).
...
where λ = the WeightingFactor value from the SPC_CHART table (0< λ<=1), ControlLimitFactor = the ControlLimitFactor value from the SPC_CHART table, and InControlStdDeviation = the InControlStdDeviation value from the SPC_CHART table.
...
Otherwise, UCLValue1 = XBar + (ControlLimitFactor * InControlStdDeviation * (√ (λ/((2 - λ) * subgroup size))))
...
- The system persists the Query, Key1, Key2, Key3, Key4, and Key5 values into the SPC_CHART_CALCULATION table and creates the SPC_CHART_DATA points for the data required. The system assigns the calculation ID from the SPC_CHART_CALCULATION table to all the chart points for the calculation. 
- The system then checks if there is a remainder subgroup or it is incomplete. If so, then this record will be created with a status of Incomplete (4). 
