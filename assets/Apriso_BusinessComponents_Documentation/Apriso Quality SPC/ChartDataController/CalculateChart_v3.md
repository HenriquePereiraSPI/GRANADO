...
| Input | UseInputtedUpperSpecificationLimit | Boolean | No | Set to True if the the upper specification limit from user Input is to be used. |
...
| Output | Maximum | Decimal | No | The maximum value of the data points. |
...
- The system checks for the specification limit calculation type. 
...
- If the value is (2), the system validates the value of CharacteristicRevisionID. If it has the value, the system uses the UpperSpecificationLimit, LowerSpecificationLimit, and TargetSpecificationLimit fields from the CHARACTERISTIC_REVISION table. If it does not have the value, the system checks the value of the Characteristic. If it has the value, the system uses the UpperSpecificationLimit, LowerSpecificationLimit, and TargetSpecificationLimit fields from CHARACTERISTIC table. If it does not have the value, the system returns an error. 
...
- The system validates Query. If it has been specified, the system uses Query for calculation. If it has not been specified, system uses the CalculateQuery field in the SPC_CHART table. If there is no CalculateQuery in the SPC_CHART table, system uses the CalculateQuery field in the SPC_CHART_TYPE table. If there is no CalculateQuery in the SPC_CHART_TYPE table, the system returns an error.  
...
RBar = sum of the subgroup ranges/total number of subgroups.
...
XBar = median of subgroup medians = the midpoint value of the subgroup medians sorted in ascending order. If there are two middle values, the mean of them is taken.
     

 **ControlValue2** 
   

RBar = median of subgroup ranges = the midpoint value of the subgroup ranges sorted in ascending order. If there are two middle values, the mean of them is taken.
...
ID of the chart from the SPC_CHART table.
...
Otherwise, LCLValue2 = Ratio3 * ControlValue2
...
XBar = Sum of Sub Group Averages/Total number of Sub Groups
     

 **PlottedValue1** 
   

For the first record, PlottedValue1 = subgroup average = totals of subgroup/subgroup size. Otherwise, PlottedValue1 = (λ * subgroup average) + ((1 - λ) * Previous PlottedValue1) where λ = WeightingFactor value from the SPC_CHART table (0< λ<=1).
...
Otherwise, LCLValue1 = XBar - (ControlLimitFactor * InControlStdDeviation * (√ (λ/((2 - λ) * subgroup size)))) where λ = WeightingFactor value from the SPC_CHART table (0< λ<=1), ControlLimitFactor = the ControlLimitFactor value from the SPC_CHART table, and InControlStdDeviation = the InControlStdDeviation value from the SPC_CHART table.
...
Otherwise, UCLValue1 = XBar + (ControlLimitFactor * InControlStdDeviation * (√ (λ/((2 - λ) * subgroup size)))) where λ = the WeightingFactor value from the SPC_CHART table (0< λ<=1), ControlLimitFactor = the ControlLimitFactor value from the SPC_CHART table, and InControlStdDeviation = the InControlStdDeviation value from the SPC_CHART table.
...
The Range, Sigma, and Average values refer to the corresponding columns in the SPC_CHART_DATA table that are associated with the SPC_CHART_CALCULATION.
 

 

The values of Ratio1 and Ratio2 depend on the size of the subgroup.
...
Min(PpU, PpL): the smaller of the two values.
...
- The system checks if there is a remainder subgroup or incomplete. If so, this record will be created with the status of Incomplete (4). 
