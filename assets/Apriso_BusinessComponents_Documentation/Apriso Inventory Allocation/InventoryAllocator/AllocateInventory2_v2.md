...
The difference between the aggregated and the full mode is visible only for such Container actions as allocate, deallocate, decrease and move with child containers. BC AllocateInventory2_v2 allocates specific row from INVENTORY2 table (based on Inventory2ID provided as input)
