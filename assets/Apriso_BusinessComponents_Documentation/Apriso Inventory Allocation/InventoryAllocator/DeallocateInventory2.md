...
| Input | OperationCode | Char | No | DFC Code for the transaction history. |
...
The difference between the aggregated and the full mode is visible only for such Container actions as allocate, deallocate, decrease and move with child containers. DeallocateInventory2 BC deallocates only a specified Container (if provided) with no impact on subcontainers.
