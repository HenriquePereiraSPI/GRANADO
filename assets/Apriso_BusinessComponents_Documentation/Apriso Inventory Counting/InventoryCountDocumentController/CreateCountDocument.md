...
Cycle Counts can be performed for a variety of reasons. The purpose of this Business Component is to provide a flexible way of creating Cycle Counting documents. 
 Depending on the inputs, one or multiple count documents is created. 
 Count Documents can be created for a location (Warehouse or Facility), or be restricted to a WarehouseLocation or a Zone; it can be created for a Product, or restricted to a given Product's Grade, Status, or PartnerID, or any combination of location level, product and inventory attributes.
 The count task level defined in the Count Definition determines the number of count documents to be created. For example, if inputs include a Warehouse Location and a Product, then a single Count Document will be created to count that Product in that Location. If the location input is a Warehouse, if the count task level is 1 then just one count document for counting the entire Warehouse. If the count task level is 2, a Count document will be created for each Warehouse Location of the inputted Warehouse.
 This BC is flexible enough to also give supervisors and counters the ability to create count documents quickly and seamlessly on the fly.
