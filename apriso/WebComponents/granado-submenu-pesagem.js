...
/* __granado_guard__ */
if (!customElements.get('granado-submenu-pesagem')) {
  class GranadoSubmenuPesagem extends HTMLElement {
    ...
    _render() {
      ...
      const backBtnHtml = backLabel ? `<button data-sm-back type="button" style="height:auto !important;min-height:0 !important;background:#FFFFFF;border:1.5px solid ${backColor};color:${backColor};padding:6px 12px;height:auto;border-radius:6px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap;line-height:1.2;transition:background 0.15s,color 0.15s">${this._escape(backLabel)}</button>` : '';
  ...
  window.GranadoSubmenuPesagem = GranadoSubmenuPesagem;
}
