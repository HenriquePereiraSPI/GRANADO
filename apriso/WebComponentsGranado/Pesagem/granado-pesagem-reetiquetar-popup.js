/* ============================================================
   ...
   ── Exemplo
   <script src="[AprisoScripts]/WebComponents/granado-pesagem-reetiquetar-popup.js"></script>
   <script>
     GranadoPesagemReetiquetarPopup.show({
...
if (!customElements.get('granado-pesagem-reetiquetar-popup')) {
  ...
  class GranadoPesagemReetiquetarPopup extends HTMLElement {
    ...
    static get observedAttributes() { return ['mobile-breakpoint', 'close-on-backdrop', 'open']; }
    ...
    static show(opts) {
      ...
      el._confirmText = opts.confirmText || '✓ Concluir';
          ...
          '<div data-role="box" style="' + boxCss + '">' +
            ...
            (m
              ? '<div style="display:flex;gap:10px;margin-top:18px;padding-top:16px;border-top:1px solid ' + BORDER + '">' +
                  ...
                  '<button data-role="confirm" type="button" style="flex:1.4;font:800 14px/1.4 ' + FONT + ';padding:12px 16px;border:1px solid ' + VERDE_ESC + ';border-radius:9px;background:' + VERDE + ';color:#fff;cursor:pointer">' + esc(this._confirmText) + '</button>' +
                '</div>'
              : '<div style="display:flex;justify-content:flex-end;gap:10px;margin-top:20px;padding-top:16px;border-top:1px solid ' + BORDER + '">' +
                  ...
                  '<button data-role="confirm" type="button" style="font:800 13px/1.4 ' + FONT + ';padding:9px 22px;border:1px solid ' + VERDE_ESC + ';border-radius:8px;background:' + VERDE + ';color:#fff;cursor:pointer">' + esc(this._confirmText) + '</button>' +
                '</div>') +
    ...
    _wire() {
      ...
      this.querySelector('[data-role="mode-toggle"]').addEventListener('click', function () {
        if (self._state.mode === 'balanca') {
          ...
          // Indo p/ balança: se a sala não tem balança, não faz sentido.
          if (self._salaSemBalanca()) {
            self._alerta('Esta sala não tem balança — use o peso manual.');
            return;
          }
      ...
      if (st.view === 'bals') {
        ...
        html =
          ...
          '<div style="display:flex;gap:8px;overflow-x:auto;padding-bottom:4px;-webkit-overflow-scrolling:touch">' +
            ((sala.bals && sala.bals.length)
              ? sala.bals.map((b) => self._balCard(b)).join('')
              : '<div style="font:600 12px/1.5 ' + FONT + ';color:' + TEXT2 + ';padding:6px 2px">Sala sem balança — use o <strong>Peso manual</strong> abaixo.</div>') +
          '</div>';
      } else {
      ...
      if (st.view === 'bals') {
        ...
        this._grpGrid.querySelectorAll('[data-sala]').forEach(function (card) {
          card.addEventListener('click', function () {
            ...
            st.balId = null;
            st.view = 'bals';
            // Sala sem balança -> já entra no Peso manual (não há balança pra ler).
            st.mode = self._salaSemBalanca() ? 'manual' : 'balanca';
            self._renderDrill();
            self._applyMode();
          });
        });
    ...
    _salaSemBalanca() {
      const sala = this._achaSala(this._state.salaId);
    ...
    _applyMode() {
      ...
      var dimGrid = !balOn && !this._salaSemBalanca();   // no manual só "trava" o grid quando há balança
      this._grpGrid.style.opacity = dimGrid ? '.45' : '1';
      this._grpGrid.style.pointerEvents = dimGrid ? 'none' : '';
